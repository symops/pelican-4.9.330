...
blk_qc_t submit_bio(struct bio *bio)
{
	...
	if (bio_has_data(bio)) {
		...
		if (unlikely(block_dump)) {
			char b[BDEVNAME_SIZE];
			static char _comm[TASK_COMM_LEN] = "";
			if (block_dump_count > 0 && strncmp(_comm, current->comm, sizeof(_comm))!=0 && strncmp(bdevname(bio->bi_bdev, b), "md", 2)==0 && strncmp(current->comm, "jbd2", 4)!=0 && strncmp(current->comm, "kworker", 7)!=0 ) {
				block_dump_count--;
				printk(KERN_DEBUG "[KML] %s(%d): %s block %Lu on %s (%u sectors)\n",
				current->comm, task_pid_nr(current),
					op_is_write(bio_op(bio)) ? "WRITE" : "READ",
					(unsigned long long)bio->bi_iter.bi_sector,
					bdevname(bio->bi_bdev, b),
					count);
				strncpy(_comm, current->comm, sizeof(_comm));
			}
		}
	}
