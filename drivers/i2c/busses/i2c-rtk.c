...
static int rtk_i2c_init_recovery_info(struct rtk_i2c_dev *priv)
{
	...
	bri->scl_gpio = of_get_gpio(np, 0);
	bri->sda_gpio = of_get_gpio(np, 1);

	if (bri->sda_gpio == -EPROBE_DEFER || bri->scl_gpio == -EPROBE_DEFER) {
		return -EPROBE_DEFER;
	} else if (!gpio_is_valid(bri->sda_gpio) ||
		   !gpio_is_valid(bri->scl_gpio)) {
		dev_err(dev, "recovery information incomplete\n");
		return 0;
	}

	bri->prepare_recovery = rtk_i2c_prepare_recovery;
	bri->unprepare_recovery = rtk_i2c_unprepare_recovery;
	bri->recover_bus = i2c_generic_gpio_recovery;
...
static int rtk_i2c_probe(struct platform_device *pdev)
{
	...
	strlcpy(priv->adap.name, "rtk i2c adapter", sizeof(priv->adap.name));
