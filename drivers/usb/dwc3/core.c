...
#if 1 // USB_PATCH_BY_RTK
	/* For some dwc3 controller do't use DWC3_GLOBALS_REGS_START (0xc100)
	 * We fixed it */
	device_property_read_u32(dev, "fixed_dwc3_globals_regs_start",
				 &dwc->fixed_dwc3_globals_regs_start);

	if (dwc->fixed_dwc3_globals_regs_start) {
		res->start -= DWC3_GLOBALS_REGS_START;
		res->start += dwc->fixed_dwc3_globals_regs_start;
		dev_info(dev,
		    "fixed dwc3 globals register start address to 0x%x\n",
...
err0:
	...
	if (dwc->fixed_dwc3_globals_regs_start) {
		...
		res->start -= dwc->fixed_dwc3_globals_regs_start;
		dev_info(dev,
		    "restore dwc3 globals register start address 0x%x\n",
		    (unsigned int)res->start);
	}
#endif // USB_PATCH_BY_RTK
