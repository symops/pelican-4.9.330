...
#define V0_vref_2p6v(val) ((0x7 & val) << 26)
...
#define V0_decode_2p6v(val) ((val >> 26) & 0x7)
...
static int detect_host(struct type_c_data *type_c)
{
	...
	if (type_c->check_at_boot || (type_c->rd_config == EXTERNAL_Rd)) {
		if ((type_c->chip_id & 0xFFF0) == CHIP_ID_RTD129X) {
			...
			dev_info(dev, "%s external Rd is build on chip RTD%x, set EN_EXTERNAL_RD Bit 29\n",
...
irqreturn_t type_c_detect_irq(int irq, void *__data)
{
	...
	if (type_c->connect_change) {
		...
		if (type_c->debug)
			dev_dbg(dev, "%s: ###NO change### Status: IN %s mode %s %s (at %s interrupt) int_status=0x%x, cc_status=0x%x",
				    __func__,
				    type_c->cc_mode?"host":"device",
				    type_c->cc_mode ?
				     (type_c->is_attach?"attach":"detach") :
...
static DEVICE_ATTR(role_swap, 0644, type_c_role_swap_show,
	    type_c_role_swap_write);

static ssize_t type_c_force_set_mode_show(struct device *dev,
			      struct device_attribute *attr,
			      char *buf)
{
	struct type_c_data *type_c = dev_get_drvdata(dev);
...
static ssize_t type_c_force_set_mode_write(struct device *dev,
			       struct device_attribute *attr,
			       const char *buf, size_t count)
{
	struct type_c_data *type_c = dev_get_drvdata(dev);
...
static DEVICE_ATTR(force_set_mode, 0644, type_c_force_set_mode_show,
	    type_c_force_set_mode_write);
...
static int type_c_set_parameter_show(struct seq_file *s, void *unused)
{
	...
	seq_puts(s, "rd_config [internal|external]");
		...
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	...
	if (!strncmp(buf, "cc_dfp_mode", 11)) {
		...
		if (ret < 0) {
			dev_err(type_c->dev, "sscanf ret error (%d)", ret);
			return -EFAULT;
		}
...
static int set_type_c_parameter(struct type_c_data *type_c, struct device *dev)
{
	struct device_node *node = dev->of_node;
	const char *str;
	u32 val, default_val;
...
static int dwc3_rtk_type_c_probe(struct platform_device *pdev)
{
	...
	if (type_c->type_c_reg_base == NULL) {
		/* For Kylin Case */
		type_c->type_c_reg_base = type_c->wrap_base + 0x14C;
	...
	if (node && of_device_is_available(node)) {
		...
		dev_info(dev, "Set device mode boot_check_time %d ms ==> (%s to check)\n",
			    type_c->boot_check_time,
	...
	if (set_type_c_parameter(type_c, dev)) {
		dev_err(dev, "ERROR: %s to set type c parameter!!", __func__);
	}
