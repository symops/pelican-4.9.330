...
struct rtk_wdt {
	const struct rtk_wdt_data *desc;
	struct device *dev;
...
static bool rtk_wdt_check_ints(struct rtk_wdt *wdt)
{
	u32 val;
	u32 offset = wdt->desc->ints_offset;
	u32 shift = wdt->desc->ints_shift;

	regmap_read(wdt->regmap, offset, &val);
	return !!(val & BIT(shift));
}

static void rtk_wdt_clear_ints(struct rtk_wdt *wdt)
{
	u32 offset = wdt->desc->ints_offset;
	u32 shift = wdt->desc->ints_shift;

	regmap_write(wdt->regmap, offset, BIT(shift) & ~0x1);
}
...
static irqreturn_t rtk_wdt_irq_handler(int irq, void *devid)
{
	...
	if(!rtk_wdt_check_ints(wdt))
		return IRQ_NONE;
	rtk_wdt_clear_ints(wdt);
...
static int rtk_wdt_reboot_notify(struct notifier_block *nb, unsigned long mode, void *cmd)
{
	...
	dev_info(wdt->dev, "set oe to %d\n", wdt->ov_rstb_oe);
	rtk_wdt_set_ov_rstb_oe(wdt, wdt->ov_rstb_oe);
	rtk_wdt_set_ov_rstb_cnt(wdt, 0x00800000);
	return NOTIFY_OK;
}
...
static int rtk_wdt_probe(struct platform_device *pdev)
{
	...
	wdt->regmap = syscon_regmap_lookup_by_phandle(dev->of_node, "syscon");
	if (IS_ERR(wdt->regmap))
		wdt->regmap = syscon_regmap_lookup_by_phandle(dev->of_node, "realtek,misc");
	if (IS_ERR(wdt->regmap)) {
		ret = PTR_ERR(wdt->regmap);
		dev_err(dev, "failed to get syscon of intctrl: %d\n", ret);
		return ret;
	}

	wdt->irq = platform_get_irq(pdev, 0);
	if (wdt->irq < 0) {
		dev_err(dev, "no irq specified: %d\n", wdt->irq);
		return wdt->irq;
	}
	...
	if (of_property_read_u32(dev->of_node, "restart_priority", &restart_priority))
		restart_priority = -1;
	watchdog_set_restart_priority(&wdt->wdd, restart_priority);
	...
	if (of_property_read_bool(dev->of_node, "realtek,start-wdt")) {
		dev_info(dev, "start watchdog\n");
...
static const struct rtk_wdt_data misc_wdt_data = {
	.ints_offset      = 0xc,
	.ints_shift       = 2,
};

static const struct rtk_wdt_data iso_wdt_data = {
	.ints_offset      = 0,
	.ints_shift       = 7,
};

static const struct rtk_wdt_data rtd1619b_iso_wdt_data = {
	.ints_offset      = 0x0,
	.ints_shift       = 4,
};

static const struct of_device_id rtk_wdt_of_table[] = {
	{ .compatible = "realtek,watchdog",          .data = &misc_wdt_data, },
	{ .compatible = "realtek,iso-watchdog",      .data = &iso_wdt_data, },
	{ .compatible = "realtek,rtd1619b-watchdog", .data = &rtd1619b_iso_wdt_data, },
	{}
};
