...
static void rtk_reset_reg_lock(struct rtk_reset_data *data)
{
	if (rtk_reset_should_locked(data))
		sb2_sem_lock(data->lock, SB2_SEM_NO_WARNING);
}

static void rtk_reset_reg_unlock(struct rtk_reset_data *data)
{
	if (rtk_reset_should_locked(data))
		sb2_sem_unlock(data->lock);
}
