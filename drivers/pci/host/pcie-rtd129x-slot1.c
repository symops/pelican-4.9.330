...
void rtk_pcie1_change_bar(u32 addr)
{
	rtk_pci_ctrl_write(0xD04, addr);
}
EXPORT_SYMBOL(rtk_pcie1_change_bar);
	...
	int reg, int size, u32 *pval)
{
	...
	if (rtk_cpu_revision == RTD_CHIP_A00)
		udelay(200);
...
static int rtk_pcie_hw_initial(struct device *dev)
{
	...
	if (rtk_cpu_revision == RTD_CHIP_A00 ||
		(rtk_cpu_id == CHIP_ID_RTD1296 &&
		rtk_cpu_revision != RTD_CHIP_B00)) {
	...
	if (rtk_cpu_revision == RTD_CHIP_A01 ||
		rtk_cpu_revision == RTD_CHIP_A00) {
	...
	rtk_pci_ctrl_write(0xC78, 0xF42401);
...
static int rtk_pcie_probe(struct platform_device *pdev)
{
	...
	rtk_cpu_id = get_rtd_chip_id();
	rtk_cpu_revision = get_rtd_chip_revision();
