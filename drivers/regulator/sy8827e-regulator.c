...
static int sy8827e_regulator_set_state(struct sy8827e_data *data,
				       int is_coldboot)
{
	...
	rstate = &data->rdev->constraints->state_mem;
	*rstate = is_coldboot ? data->state_coldboot : data->state_suspend;
	...
	if (rstate->enabled && rstate->uV) {
		...
		dev_dbg(dev, "set sleep_mode to volt-%d\n",
			rstate->uV);
		vsel = regulator_map_voltage_linear(rdev, rstate->uV,
						    rstate->uV);
		if (vsel < 0)
			return -EINVAL;
...
static int sy8827e_regulator_suspend(struct device *dev)
{
	...
	sy8827e_regulator_set_state(data, 0);
	dev_info(dev, "exit %s\n", __func__);
	return 0;
}
	...
	const struct i2c_device_id *id)
{
	...
	regmap = devm_regmap_init_i2c(client, &sy8827e_regmap_config);
	if (IS_ERR(regmap))
		return -EINVAL;
	...
	if (ret) {
		dev_err(dev, "failed to init regmap fields: %d\n", ret);
		return ret;
	}
...
static void sy8827e_regulator_shutdown(struct i2c_client *client)
{
	...
	dev_info(dev, "enter %s\n", __func__);
	sy8827e_regulator_set_state(data, 1);
	dev_info(dev, "exit %s\n", __func__);
}
