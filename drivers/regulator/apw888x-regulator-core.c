...
static int apw888x_set_clamp_en(struct regulator_dev *rdev,
					  int enable)
{
	struct apw888x_regulator_data *data = rdev_get_drvdata(rdev);
						    ...
						    unsigned sel)
{
	...
	if (data->clamp)
		apw888x_set_clamp_en(rdev, sel < 0x34);
...
static ssize_t regulator_shutdown_uV_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct regulator_dev *rdev = dev_get_drvdata(dev);
	struct apw888x_regulator_data *data = rdev_get_drvdata(rdev);

	return sprintf(buf, "%d\n", data->state_shutdown .uV);
}
static DEVICE_ATTR(shutdown_microvolts, 0444, regulator_shutdown_uV_show, NULL);

static ssize_t regulator_shutdown_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct regulator_dev *rdev = dev_get_drvdata(dev);
	struct apw888x_regulator_data *data = rdev_get_drvdata(rdev);
	int state = data->state_shutdown.enabled;
...
static DEVICE_ATTR(shutdown_state, 0444, regulator_shutdown_state_show, NULL);

static struct attribute *apw888x_regulator_dev_attrs[] = {
	&dev_attr_shutdown_microvolts.attr,
	&dev_attr_shutdown_state.attr,
	NULL
};
			       ...
			       struct apw888x_regulator_desc *desc)
{
	...
	data->clamp = create_regmap_field(regdev, desc->clamp_reg,
					  desc->clamp_mask);
	...
	ret = sysfs_create_group(&data->rdev->dev.kobj, &apw888x_regulator_dev_group);
	if (ret)
		return ret;

	apw888x_set_clamp_en(data->rdev, 1);
...
int apw888x_regulator_device_generic_suspend(struct apw888x_regulator_device *regdev)
{
	...
	list_for_each_entry(pos, &regdev->list, list)
		apw888x_regulator_set_sleep_state(pos->rdev, 0);
	return 0;
}
...
int apw888x_regulator_device_generic_resume(struct apw888x_regulator_device *regdev)
{
	...
	list_for_each_entry(pos, &regdev->list, list)
		apw888x_regulator_resume(pos->rdev);
	return 0;
}
