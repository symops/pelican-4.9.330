...
static void rtd1295_pwm_get_state(struct pwm_chip *chip, struct pwm_device *pwm,
				  struct pwm_state *state)
{
	...
	state->period     = DIV_ROUND_UP(1000000000, real_freq);
	state->duty_cycle = DIV_ROUND_UP(state->period * (pc->clk_duty[id] + 1),
					 pc->clkout_div[id] + 1);
	state->enabled    = pc->enable[id];

	pr_debug("%s: csd=%d, ocd=%d, cd=%d, period=%d, duty_cycle=%d, %%=%d\n", __func__,
		pc->clksrc_div[id], pc->clkout_div[id], pc->clk_duty[id],
		state->period, state->duty_cycle,
		(int)DIV_ROUND_CLOSEST_ULL((u64)state->duty_cycle * 100, state->period));
}
...
static const struct pwm_ops rtd1295_pwm_ops = {
	...
	.get_state = rtd1295_pwm_get_state,
	.owner = THIS_MODULE,
};
