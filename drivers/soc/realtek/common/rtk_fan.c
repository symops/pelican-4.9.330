...
static int rtk_fan_compute_speed(struct rtk_fan_data *fan_data) {
	...
	uint64_t factor = TIMER_CLK90K * 60; /* second to minute */
	uint64_t speed;
	...
	factor = div_u64(factor, fan_data->fan_factor);

	/* For example */
	/* speed is RPM */
	/* speed = ((counter_value / fan_factor) / (timer / TIMER_CLK90K)) * 60 */

	speed = div_u64((counter_value * factor), timer_target);

	fan_data->speed_rpm = speed;

	dev_dbg(fan_data->dev, "%s speed =%llu (RPM)\n", __func__, speed);
	dev_dbg(fan_data->dev, "%s timer_target=%d, counter_value=%d factor=%lld\n",
		    __func__, timer_target, counter_value, factor);
...
static int rtk_fan_init(struct rtk_fan_data *fan_data, struct platform_device *pdev) {
	...
	ret = of_property_read_u32(node, "fan_factor",
					&(fan_data->fan_factor));
	if (ret < 0) {
		dev_info(dev, "%s NO set speed_factor!", __func__);
		fan_data->fan_factor = 2;
	}
