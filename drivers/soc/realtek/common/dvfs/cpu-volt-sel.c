...
static int prop_from_fss(char *name, size_t size)
{
	struct device_node *np;
	unsigned int val;
	int ret = 0;
...
static int cpu_volt_set_init_prop_name(void)
{
	...
	ret = prop_from_fss(prop_name, sizeof(prop_name));
	if (ret == -ENOTSUPP)
...
static void cpu_volt_sel_set_prop_name(void)
{
	...
	dev_pm_opp_set_prop_name(dev, prop_name);
	return;
}
...
static void cpu_volt_sel_put_opp_table(int last_cpu)
{
	struct device *dev;
	/*
	 * the opp table will be transfer to the last cpu on cpufreq exit
	 */
	int cpu = last_cpu ? cpumask_last(cpu_possible_mask) :
		                           ...
		                           void *data)
{
	...
	case BUS_NOTIFY_UNBOUND_DRIVER:
		cpu_volt_sel_put_opp_table(1);
		return NOTIFY_OK;
	case BUS_NOTIFY_DRIVER_NOT_BOUND:
		cpu_volt_sel_put_opp_table(0);
		return NOTIFY_OK;
	}
	return NOTIFY_DONE;
}
