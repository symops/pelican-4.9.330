...
static inline bool port_is_enable(struct port_data *port)
{
	bool enable = true;
...
int RTK_usb_reprobe_usb_storage(struct usb_device *udev)
{
	 ...
	 * [SOLUTION] dont unbind and probe again after usb_add_device,
	 * and set it as quirk device
	 * commit 75b83399dcf5e38ae39ce16d730f5824fccbb3bf
	 * [DEV_FIX]xhci hub 2.0  can't be recognize after
	 * unplug and plug again
	 * commit fbb4cc12d549b5193d51e88d6d0bc916a6e4d5d3
	 * [ROOT_CAUSE] USB hdd can't be mounted cuz scsi not stable
	...
	if (udev->actconfig) {
		...
		for (i = 0; i < 1/*udev->actconfig->desc.bNumInterfaces*/;
			    i++) {
			...
			dev_info(&udev->dev, "bInterfaceClass = %d (jiffies 0x%lx = %d ms)\n",
	...
	if ((jiffies - INITIAL_JIFFIES) < 0x400
		...
		msleep(100);
		dev_info(&udev->dev, "%s unbind-probe dev_name = %s (jiffies= 0x%lx = %d ms)\n",
			    __func__,
		...
		dev_info(&udev->dev, "%s bind-probe dev_name = %s (jiffies= 0x%lx = %d ms)\n",
			    __func__,
			    dev_name(&udev->dev), jiffies - INITIAL_JIFFIES,
			    jiffies_to_msecs(jiffies - INITIAL_JIFFIES));
...
{
	static struct manager_data *s_data;
...
static int __usb_init_clock_reset(struct manager_data *data)
{
	...
	/* GET clock */
	struct clk *clk_usb_phy0_to_host = USB_clk_get(dev,
		    "clk_en_phy0_to_host");
	struct clk *clk_usb_phy1_to_host = USB_clk_get(dev,
		    "clk_en_phy1_to_host");
	struct clk *clk_usb_phy2_to_host = USB_clk_get(dev,
		    "clk_en_phy2_to_host");
	struct clk *clk_usb_phy3_to_host = USB_clk_get(dev,
		    "clk_en_phy3_to_host");
	...
	struct reset_control *reset_u3phy2 = USB_reset_get(dev, "u3phy2");
	struct reset_control *reset_u3phy3 = USB_reset_get(dev, "u3phy3");
	...
	struct reset_control *reset_u3phy2_mdio = USB_reset_get(dev, "u3mdio2");
	struct reset_control *reset_u3phy3_mdio = USB_reset_get(dev, "u3mdio3");
	...
	USB_reset_assert(reset_u3phy2_mdio);
	USB_reset_assert(reset_u3phy3_mdio);
	...
	if (port_is_enable(&data->port2))
		USB_reset_deassert(reset_u3phy2_mdio);
	if (port_is_enable(&data->port3))
		USB_reset_deassert(reset_u3phy3_mdio);

	USB_reset_deassert(reset_usb_apply);

	dev_dbg(dev, "Realtek USB init: Set usb reset to 1\n");

	/* port0 mac's reset bit must always enable */
	...
	USB_reset_put(reset_u3phy2_mdio);
	USB_reset_put(reset_u3phy3_mdio);
	    ...
	    struct gpio_data *gpio, bool on)
{
	...
	if (on) {
		...
		if ((gpio->enable_port & enable_port_mask) !=
			    enable_port_mask) {
			dev_info(dev, "gpio_num=%d enable_port=0x%x enable_port_mask=0x%x  ==> No turn on\n",
				    gpio->gpio_num, gpio->enable_port,
				    enable_port_mask);
			return -1;
		}
	} else {
		...
		if (gpio->enable_port != 0) {
			dev_info(dev, "gpio_num=%d enable_port=0x%x enable_port_mask=0x%x  ==> No turn off\n",
				    gpio->gpio_num, gpio->enable_port,
				    gpio->enable_port_mask);
			return -1;
		}
	}
	...
	if (gpio_is_valid(gpio_num)) {
		if (gpio_request(gpio_num, "usb_power_gpio")) { //request gpio
			...
			if (gpio_direction_output(gpio_num,
...
static int rtk_port0_power_show(struct seq_file *s, void *unused)
{
	...
	seq_puts(s, "To Control Port 0\n");
	seq_puts(s, "echo \"on\" > port0_power\n");
	seq_puts(s, "echo \"off\" > port0_power\n");
	seq_puts(s, "\n");
	if (data->port0.power_gpio) {
		...
		seq_puts(s, "port0 No control gpio\n");
	}
...
static int rtk_port1_power_show(struct seq_file *s, void *unused)
{
	...
	seq_puts(s, "To Control Port 1\n");
	seq_puts(s, "echo \"on\" > port1_power\n");
	seq_puts(s, "echo \"off\" > port1_power\n");
	seq_puts(s, "\n");
	if (data->port1.power_gpio) {
		...
		seq_puts(s, "port1 No control gpio\n");
	}
	return 0;
}
...
static int rtk_port2_power_show(struct seq_file *s, void *unused)
{
	...
	seq_puts(s, "To Control Port 2\n");
	seq_puts(s, "echo \"on\" > port2_power\n");
	seq_puts(s, "echo \"off\" > port2_power\n");
	seq_puts(s, "\n");
	if (data->port2.power_gpio) {
		...
		seq_puts(s, "port2 No control gpio\n");
	}
	return 0;
}
...
static int rtk_port3_power_show(struct seq_file *s, void *unused)
{
	...
	seq_puts(s, "To Control Port 3\n");
	seq_puts(s, "echo \"on\" > port3_power\n");
	seq_puts(s, "echo \"off\" > port3_power\n");
	seq_puts(s, "\n");
	if (data->port3.power_gpio) {
		...
		seq_puts(s, "port3 No control gpio\n");
	}
	return 0;
}
...
static void rtk_usb_platform_init(struct manager_data *data)
{
	data->rtk_usb = kzalloc(sizeof(struct rtk_usb_ops), GFP_KERNEL);

	if ((data->chip_id & 0xFFF0) == CHIP_ID_RTD129X)
		rtk_usb_rtd129x_init(data->rtk_usb);
	else
		dev_info(data->dev, "%s No support this chip_id=%x\n",
...
static int rtk_usb_manager_probe(struct platform_device *pdev)
{
	...
	data->chip_id = get_rtd_chip_id();
	rtk_usb_platform_init(data);
