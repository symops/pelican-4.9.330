...
#include "rtk_rpc.h"
#ifdef CONFIG_ION_RTK  /*ion is unavailable if RTD129x purenas*/
#include "uapi/ion.h"
#include "ion/ion.h"
...
static DEFINE_MUTEX(rpc_alloc_lock);
...
{
	int ret = mutex_lock_interruptible(&rpc_alloc_lock);
	if (ret) {}
	entry->next = r_program_head;
	r_program_head = entry;
	r_program_count++;
	mutex_unlock(&rpc_alloc_lock);
}
...
static r_program_entry_t *r_program_remove(unsigned long phys_addr)
{
	...
	ret = mutex_lock_interruptible(&rpc_alloc_lock);
	curr = r_program_head;
	while (curr != NULL) {
...
ssize_t r_program_write(int opt, RPC_DEV_EXTRA *extra, RPC_DEV *dev, char *buf, size_t count)
{
	...
	ssize_t ret = 0;
	uint32_t ptmp;
	__maybe_unused int rpc_ring_size = dev->ringEnd - dev->ringStart;
	RPC_STRUCT *rpc;
...
void rpc_ion_handler(RPC_DEV_EXTRA *extra)
{
	...
	RPC_STRUCT *rpc;
	RPC_STRUCT *rrpc;
	__maybe_unused unsigned long map_addr;
...
#define FW_ALLOC_VCPU_EXTRA 0x80000000
...
#else
                        (ION_FLAG_NONCACHED  | ION_FLAG_HWIPACC | ION_FLAG_ACPUACC)
#endif
                        ;
                    ...
                    if (IS_ERR(handle)) {
                        pr_err("[%s] ERROR: ion_alloc fail %d(heap:0x%x flags:0x%x)\n", __func__, (int)(long)(int *) handle, ion_alloc_heap_mask, ion_alloc_flags);
                        reply_value = -1U;
                        break;
                    }
                    ...
                    pr_debug("[%s] ion_alloc addr : 0x%x (heap:0x%x flags:0x%x)\n", __func__, (int)reply_value, ion_alloc_heap_mask, ion_alloc_flags);
...
void rpc_dispatch(unsigned long data)
{
...
#if CONFIG_ION_RTK  /*ion is unavailable if RTD129x purenas*/   /*audio fw ion remote alloc handled by kernel*/
	...
	case REPLYID:
		if (rpc.versionID == REPLYID &&
			...
			taskID = ntohl(taskID);
			pr_debug("[%s]Reply: %s:%d taskID:%u\n", __func__, extra->name, __LINE__, taskID);
...
int rpc_intr_init(void)
{
	...
	int j = 0;
#ifdef CONFIG_ION_RTK  /*ion is unavailable if RTD129x purenas*/
	__maybe_unused struct ion_handle *handle = NULL;
	__maybe_unused ion_phys_addr_t phys_addr;
#endif
	__maybe_unused size_t alloc_val;
	is_init = 0;
#ifdef CONFIG_ION_RTK  /*ion is unavailable if RTD129x purenas*/
...
int rpc_intr_release(struct inode *inode, struct file *filp)
{
...
#endif /* CONFIG_REALTEK_RPC_PROGRAM_REGISTER */
	__maybe_unused __maybe_unused RPC_THREAD *th, *thtmp;
	int minor = MINOR(inode->i_rdev);
	...
	pr_info("self destroy: %s extra->name:%s\n", current->comm, extra->name);

	if(!strcmp(extra->name, "AudioIntrRead") || !strcmp(extra->name, "AudioIntrWrite")) {
		RPC_DESTROY_AUDIO_FLOW(current->tgid);
	}
#ifdef CONFIG_ARCH_RTD13xx
	else if (!strcmp(extra->name, "Video1IntrRead") ||!strcmp(extra->name, "Video1IntrWrite")  ) {
		RPC_DESTROY_VIDEO_FLOW(current->tgid);
	}
#endif
        break;
    } while (0);
		...
		loff_t *f_pos)
{
	...
	RPC_DEV *dev = proc->dev; /* the first listitem */
	RPC_DEV_EXTRA *extra = proc->extra;
	__maybe_unused RPC_DEV_EXTRA *rextra = extra + 1;
	int temp, size;
	size_t r;
	ssize_t ret = 0;
	uint32_t ptmp;
