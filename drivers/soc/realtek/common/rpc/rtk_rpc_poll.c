...
static struct rpc_debug_flag_memory * get_debug_flag_memory(void)
{
    ...
    do {
        if (mDebugFlagMemory == NULL) {
            ...
            ion_flag_mask = ION_FLAG_NONCACHED | ION_USAGE_MMAP_NONCACHED;
...
static struct rpc_debug_print_memory * get_debug_print_memory(void)
{
	...
	do {
		if (mDebugPrintMemory == NULL) {
			...
			tmp->handle = ion_alloc(tmp->client, 1024*64, 0 /*align*/,
				RTK_PHOENIX_ION_HEAP_AUDIO_MASK, ion_flag_mask);
			...
			tmp->debug_hdr = (void *) ion_map_kernel(tmp->client, tmp->handle);
...
long rpc_ctrl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case RPC_IOCTRGETDBGREG_V:
		...
		if (copy_from_user(&dFlag, (void __user *)arg, sizeof(dFlag)))
            ...
            if (debug_flag == NULL || debug_flag_phyAddr == -1UL)
                return -EFAULT;
           ...
           break;
            ...
            pr_debug("RPC_DEBUG cmd=%s op=%s phyAddr=0x%08lx flag=0x%08lx",
                    ...
                    debug_flag_phyAddr, (long)*puDebugFlag);
        }
        break;
#endif
#ifdef CONFIG_ION_RTK /*ion is unavailable if RTD1295 purenas*/
	...
	if (copy_from_user(&dFlag, (void __user *)arg, sizeof(dFlag))) {
            ...
            __maybe_unused unsigned int * puDebugPrint = NULL;
            struct rpc_debug_print_memory * debug_print = get_debug_print();
            phys_addr_t debug_print_phyAddr = get_debug_print_phyAddr();

            if (debug_print == NULL || debug_print_phyAddr == -1UL)
                return -EFAULT;

            dFlag.flagAddr = (uint32_t) debug_print_phyAddr & -1U;
            dFlag.flagValue = ion_share_dma_buf_fd(debug_print->client, debug_print->handle);
