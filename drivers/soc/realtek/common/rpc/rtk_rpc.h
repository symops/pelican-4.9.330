...
extern struct refclk_device *refclk;
static inline void show_rpc_struct(const char *func, RPC_STRUCT *rpc)
{
	...
	pr_debug("%s: program:%u version:%u procedure:%u taskID:%u sysPID:%u size:%u context:%x 90k:u %s\n",
			...
			rpc->mycontext, (u32)refclk_get_counter(refclk), in_atomic() ? "atomic" : "");
#endif	/* RPC_SUPPORT_MULTI_CALLER_SEND_TID_PID */
}
...
int RPC_DESTROY_VIDEO_FLOW(int pid);
#endif
...
#define RPC_IOCTRGETDBGPRINT_V _IOWR(RPC_IOC_MAGIC, 0x12, RPC_DBG_FLAG)
