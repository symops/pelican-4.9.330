...
static int __maybe_unused rtk_rpc_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(refclk)) {
		ret = PTR_ERR(refclk);
		if (ret == -EPROBE_DEFER) {
			dev_dbg(&pdev->dev, "refclk not ready, retry\n");
			return ret;
		} else {
			dev_err(&pdev->dev, "failed to get refclk: %d\n", ret);
			return ret;
		}
	}
...
#ifdef CONFIG_ARCH_RTD13xx
	writel((RPC_INT_SA | RPC_INT_SV | RPC_INT_WRITE_1), rpc_int_base + RPC_SB2_INT_EN);
#endif
...
static int rtk_rpc_pm_suspend(struct device *dev)
{
	...
	while ((rpc_get_flag(RPC_VIDEO) != 0x0) && (MaxCount > 0)) {
		mdelay(1);
		MaxCount--;
	}
...
#ifdef CONFIG_ARCH_RTD13xx
	RPC_RESET_BIT(rpc_vcpu_int_flag, RPC_VIDEO_SET_NOTIFY); /* Disable Interrupt */
#endif /* CONFIG_ARCH_RTD13xx */
...
#ifdef CONFIG_ARCH_RTD13xx
	rpc_set_flag(RPC_VIDEO, 0xdeadffff); /* WAIT VIDEO RPC SUSPEND READY */
	while ((rpc_get_flag(RPC_VIDEO) != 0x0) && (MaxCount > 0)) {
		mdelay(1);
		MaxCount--;
	}
...
#ifdef CONFIG_ARCH_RTD13xx
int rtk_rpc_pm_suspend_ex(int cpu_type)
{
	int MaxCount = 500;

	if (cpu_type == 2) {
		/* STOP AUDIO HAS_CHECK */
		rpc_set_flag(RPC_AUDIO, 0xdaedffff);
		while ((rpc_get_flag(RPC_AUDIO) != 0x0) && (MaxCount > 0)) {
			mdelay(1);
			MaxCount--;
		}
		...
		rpc_set_flag(RPC_AUDIO, 0xdeadffff);
		while ((rpc_get_flag(RPC_AUDIO) != 0x0) && (MaxCount > 0)) {
			mdelay(1);
			MaxCount--;
		}
		...
		/* STOP VIDEO HAS_CHECK */
		rpc_set_flag(RPC_VIDEO, 0xdaedffff);
		while ((rpc_get_flag(RPC_VIDEO) != 0x0) && (MaxCount > 0)) {
			mdelay(1);
			MaxCount--;
		}
		...
		rpc_set_flag(RPC_VIDEO, 0xdeadffff);
		while ((rpc_get_flag(RPC_VIDEO) != 0x0) && (MaxCount > 0)) {
			mdelay(1);
			MaxCount--;
		}
...
EXPORT_SYMBOL(rtk_rpc_pm_suspend_ex);

int rtk_rpc_pm_resume_ex(int cpu_type)
{
	if (cpu_type == 2) {
		RPC_SET_BIT(rpc_acpu_int_flag, RPC_AUDIO_SET_NOTIFY);
		rpc_set_flag(RPC_AUDIO, 0xffffffff);
...
EXPORT_SYMBOL(rtk_rpc_pm_resume_ex);
#endif /* CONFIG_ARCH_RTD13xx */
...
static int rtk_rpc_pm_resume(struct device *dev)
{
	...
	RPC_SET_BIT(rpc_vcpu_int_flag, RPC_VIDEO_SET_NOTIFY);
	rpc_set_flag(RPC_VIDEO, 0xffffffff);
	writel((RPC_INT_SA | RPC_INT_SV | RPC_INT_WRITE_1), rpc_int_base +
RPC_SB2_INT_EN);
#endif /* CONFIG_ARCH_RTD13xx */
