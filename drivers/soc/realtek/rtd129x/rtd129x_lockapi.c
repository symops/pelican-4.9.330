...
static int get_rtd1295_chip_revision(void)
{
	void __iomem *chip_revision_addr = ioremap(0x9801A204, 0x4);
	int rev;

	rev = readl(chip_revision_addr);
	__iounmap(chip_revision_addr);

	switch (rev) {
	case (RTD129x_CHIP_REVISION_A00):
		return RTD_CHIP_A00;
	case (RTD129x_CHIP_REVISION_A01):
		return RTD_CHIP_A01;
	case (RTD129x_CHIP_REVISION_B00):
		return RTD_CHIP_B00;
	case (RTD129x_CHIP_REVISION_B01):
		return RTD_CHIP_B01;
	default:
		break;
	}
...
{
	ichip_id = 0;//get_rtd_chip_id();
	ichip_revision = get_rtd1295_chip_revision();
	locakapi_counter = 0;
	locakapi_lcounter = 0;
	iemmc_flag = 0;
	memset( caller_name1, 0, MAX_LOG_MSG_LEN );
	...
	// ID: 0x9801a200
	// Rev.: 0x9801a204
	if( ichip_revision >= (int)RTD_CHIP_B00 )
	{
