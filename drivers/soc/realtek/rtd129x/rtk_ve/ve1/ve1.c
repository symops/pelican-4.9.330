...
__maybe_unused static int vpu_alloc_dma_buffer2(vpudrv_buffer_t *vb)
{
	unsigned int ret;
...
static long vpu_ioctl(struct file *filp, u_int cmd, u_long arg)
{
	...
	{
		vpu_clock_info_t clockInfo;
		__maybe_unused unsigned int value;
		struct clk *s_vpu_clk;
...
static int vpu_mmap(struct file *fp, struct vm_area_struct *vm)
{
	...
	if (vm->vm_pgoff == 0)
    {
		if ((vm->vm_end-vm->vm_start) != ALIGN(s_instance_pool.size, PAGE_SIZE))
        {
            pr_err("%s [%d]%s.invalid mmap size:%d for vm_pgoff:0x%lx.vm_start:0x%x.vm_end:0x%x\n",DEV_NAME,__LINE__,__func__,
                (vm->vm_end-vm->vm_start),vm->vm_pgoff,vm->vm_start,vm->vm_end);
            return -EINVAL;
        }
    ...
    {
        if (((vm->vm_pgoff == (s_vpu_register.phys_addr>>PAGE_SHIFT)) && ((vm->vm_end-vm->vm_start) != ALIGN(s_vpu_register.size, PAGE_SIZE))) || ((vm->vm_pgoff == (s_bond_register.phys_addr>>PAGE_SHIFT)) && ((vm->vm_end-vm->vm_start) != ALIGN(s_bond_register.size, PAGE_SIZE))))
        {
            pr_err("%s [%d]%s.invalid mmap size:%d for vm_pgoff:0x%lx(<<%d=0x%lx).vm_start:0x%x.vm_end:0x%x\n",DEV_NAME,__LINE__,__func__,
                (vm->vm_end-vm->vm_start),vm->vm_pgoff,PAGE_SHIFT,(vm->vm_pgoff<<PAGE_SHIFT),vm->vm_start,vm->vm_end);
            return -EINVAL;
        }
    ...
    // check if mmap target is s_common_memory and if size matched
    if ((s_common_memory.phys_addr == (vm->vm_pgoff<<PAGE_SHIFT)) && (s_common_memory.size == (vm->vm_end-vm->vm_start)))
    {
        allowMap = 1;
    }
    else
    {
        // check if mmap targe is allocated from VDI_IOCTL_ALLOCATE_PHYSICAL_MEMORY and if size matched
        spin_lock(&s_vpu_lock);
        list_for_each_entry_safe(vbp, n, &s_vbp_head, list)
        {
            //DPRINTK("%s [%d]%s.vbp(0x%px,0x%lx,%d).fp:0x%px.vm(0x%lx,%d)\n",DEV_NAME,__LINE__,__func__,vbp->filp,vbp->vb.phys_addr,vbp->vb.size,fp,(vm->vm_pgoff<<PAGE_SHIFT),(vm->vm_end-vm->vm_start));
            if ((vbp->filp == fp) && (vbp->vb.phys_addr == (vm->vm_pgoff<<PAGE_SHIFT)) && (ALIGN(vbp->vb.size, PAGE_SIZE) == (vm->vm_end-vm->vm_start)))
            {
                //DPRINTK("%s [%d]%s.mmap phys_addr:0x%lx.size:%d\n",DEV_NAME,__LINE__,__func__,vbp->vb.phys_addr,vbp->vb.size);
                allowMap = 1;
                break;
            }
        }
    ...
    {
        pr_err("%s [%d]%s.invalid mmap size:%d for phys_addr:0x%lx.vm_start:0x%x.vm_end:0x%x\n",DEV_NAME,__LINE__,__func__,
            (vm->vm_end-vm->vm_start),(vm->vm_pgoff<<PAGE_SHIFT),vm->vm_start,vm->vm_end);
        return -EINVAL;
    }
