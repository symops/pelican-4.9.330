...
static void rtk_sata_init(struct ahci_host_priv *hpriv)
{
	...
	} else if ((ahci_dev->chip_id & 0xFFF0) == CHIP_ID_RTD129X) {
		dev_info(ahci_dev->dev, "rx error select to mac original\n");
		writel(0x3c300, mmio + 0xf20);
	}
...
static ssize_t sata0_present_print_state(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct ata_host *host = dev_get_drvdata(dev);
	struct ahci_host_priv *hpriv = host->private_data;
	struct ahci_rtk_dev *ahci_dev = hpriv->plat_data;
	struct ahci_port_desc *desc = ahci_dev->ports[0];
	int ret = 0;

	ret = gpio_get_value(desc->present_io);

	return sprintf(buf, "%d\n", ret);
}

static ssize_t sata1_present_print_state(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct ata_host *host = dev_get_drvdata(dev);
	struct ahci_host_priv *hpriv = host->private_data;
	struct ahci_rtk_dev *ahci_dev = hpriv->plat_data;
	struct ahci_port_desc *desc = ahci_dev->ports[1];
	int ret = 0;
...
static DEVICE_ATTR(SATA0_present, 0444, sata0_present_print_state, NULL);
static DEVICE_ATTR(SATA1_present, 0444, sata1_present_print_state, NULL);
...
static int ahci_rtk_probe(struct platform_device *pdev)
{
	...
	for (i=0; i<MAC_MAX_RST; i++) {
		...
		if (IS_ERR(rst))
			break;
		ahci_dev->rsts[i] = rst;
		pr_info("[KML] sata ahci reset assert %d\n", i);
	...
	for_each_available_child_of_node(dev->of_node, child) {
		...
		present_io = of_get_gpio(child, 1);
		if (present_io >= 0) {
			gpio_request(present_io, child->name);
			gpio_direction_input(present_io);
			desc->present_io = present_io;
			if (id == 0)
				device_create_file(dev, &dev_attr_SATA0_present);
			else if (id == 1)
				device_create_file(dev, &dev_attr_SATA1_present);
		}
		...
		for (i=0; i<MAC_MAX_RST; i++) {
			...
			if (IS_ERR(rst))
				break;
			desc->rsts[i] = rst;
			pr_info("[KML] sata ahci port reset assert %d\n", i);
			reset_control_assert(rst);
			reg = readl(crtreg);
			dev_info(dev, "[KML] port%d crt register = 0x%x\n", id, reg);
			reset_control_deassert(rst);
			reg = readl(crtreg);
			dev_info(dev, "[KML] port%d crt register = 0x%x, after deassert\n", id, reg);
		}
		iounmap(crtreg);
		desc->index = id;
	...
	/* sync with kernel4.1.7 sata driver */
