...
static int rtk_uio_bypass;
static const char *allowed_prefixes[] = {
	"reg",
	"BandwidthMonitor",
};

static ssize_t allowed_prefix_show(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	int len = 0;
	int i;

	for (i = 0; i < ARRAY_SIZE(allowed_prefixes); i++) {
		len += snprintf(buf + len, PAGE_SIZE - len, "%s%s",
			len == 0 ? "" : ", ", allowed_prefixes[i]);
	}
	len += snprintf(buf + len, PAGE_SIZE - len, "\n");
	return len;
}
DEVICE_ATTR_RO(allowed_prefix);
...
static struct attribute *rtk_uio_attrs[] = {
	&dev_attr_allowed_prefix.attr,
	&dev_attr_bypass.attr,
	NULL
};

static struct attribute_group rtk_uio_attr_group = {
	.name = "perm",
	.attrs = rtk_uio_attrs,
};

static int rtk_uio_comm_is_allowed(void)
{
	int i, len;
	...
	for (i = 0; i < ARRAY_SIZE(allowed_prefixes); i++) {
		len = strlen(allowed_prefixes[i]);
		if (len > (TASK_COMM_LEN - 1))
			len = TASK_COMM_LEN - 1;
		if (!strncmp(current->comm, allowed_prefixes[i], len))
			return 1;
	}
	return 0;
}

static int rtk_uio_rbus_open(struct uio_info *info, struct inode *inode)
{
	struct rtk_uio_data *priv = to_data(info);

	if (!rtk_uio_comm_is_allowed()) {
		dev_info(priv->dev, "%s (pid %d) is not allowed\n", current->comm,
			 task_pid_nr(current));
		return -EPERM;
	}

	return rtk_uio_open(info, inode);
}
...
static int rtk_uio_probe(struct platform_device *pdev)
{
	...
	if (!strncmp(np->name, "rbus", 4)) {
		rtk_uio_bypass = 1;
		info->open    = rtk_uio_rbus_open;
		ret = sysfs_create_group(&dev->kobj, &rtk_uio_attr_group);
		if (ret)
			dev_warn(dev, "failed to create perm sysfs: %d\n", ret);
	}
