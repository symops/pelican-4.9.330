...
void card_stop(struct rtkemmc_host *emmc_port)
{
	...
	if (get_rtd_chip_revision() >= RTD_CHIP_A01 &&
		(get_rtd_chip_revision() < RTD_CHIP_B00 || (emmc_port->mmc->caps2 & MMC_CAP2_HS400_1_8V)==MMC_CAP2_HS400_1_8V)) {
...
//	printk(KERN_ERR "crd_tmp_buffer=%p, emmc_port->dma_paddr=%p, gddr_dma_org=%p\n",crd_tmp_buffer,emmc_port->dma_paddr,gddr_dma_org);
	...
	if (cmd_info.cmd->data == NULL)
        ...
        ret_err = SD_Stream_Cmd(EMMC_AUTOWRITE1, &cmd_info, bIgnore);
        if (ret_err)
        {
#ifdef EMMC_DEBUG
	...
	if (emmc_port->tx_tuning) {
		if (mode == MODE_DDR && flag==1)
			...
			writel(readl(emmc_port->emmc_membase+EMMC_OTHER1)|0x8,emmc_port->emmc_membase+EMMC_OTHER1);
                        ...
                        printk("phase =0x%x \n", i);
#endif
			rtkemmc_send_cmd13(emmc_port, &state);
			if((emmc_port->rintsts&0x100) != 0)
...
rx_retry:
		...
		if(RX_window==0xffffffff) {
			...
			switch(loop_cnt) {
				...
				default:
					if(loop_cnt>60)
						printk(KERN_ERR "loop cnt %d: RX_WINDOW = 0x%08x, cannot find a proper rx phase\n", loop_cnt, RX_window);
			}
...
tx1_retry:
			...
			if(((TX_window>>i)&1)==1) {
				...
				printk("phase =0x%x \n", i);
#endif
				if(rtkemmc_send_cmd25(emmc_port, 1024,0xfe,0,NULL,1) != 0)
					TX1_window= TX1_window&(~(1<<i));
			}
		}
		TX1_best = search_best(TX1_window, range);
		if(flag==1)printk(KERN_ERR "TX1_WINDOW = 0x%08x TX1_best=0x%x\n", TX1_window,TX1_best);
		if((mode == MODE_HS400 && TX1_window==0xffff) || (mode == MODE_HS200 && TX1_window==0xffffffff)) {
			if(mode == MODE_HS400)
				rtkemmc_set_pad_driving(emmc_port, pddrive_nf_s3[1]-1, pddrive_nf_s3[2]-1, pddrive_nf_s3[3]-1, pddrive_nf_s3[4]);
			else if(mode == MODE_HS200)
				rtkemmc_set_pad_driving(emmc_port, pddrive_nf_s2[1]-1, pddrive_nf_s2[2]-1, pddrive_nf_s2[3]-1, pddrive_nf_s2[4]);
			goto tx1_retry;
		}
		else {
			if(mode == MODE_HS400)
...
//	unsigned long flags2;
	...
	/*there is a IC bug for cmd23 method if L4 gated is enabled*/
#if defined(CONFIG_ARCH_RTD139x) || defined(CONFIG_ARCH_RTD16xx)
	rtkemmc_writel(readl(emmc_port->emmc_membase+EMMC_OTHER1)&0xa|0x4, emmc_port->emmc_membase+EMMC_OTHER1);    //Disable eMMC L4 gated
	...
	if (emmc_port->int_waiting) {
		...
		if (CMD_IDX_MASK(cmd_idx)==MMC_WRITE_MULTIPLE_BLOCK || CMD_IDX_MASK(cmd_idx)==MMC_READ_MULTIPLE_BLOCK ){
			if (emmc_port->rpmb_cmd) {
				...
				int_mask = readl(emmc_port->emmc_membase+EMMC_RINTSTS);
				timeend = jiffies + msecs_to_jiffies(msec);
				while((int_mask & 0x4) == 0)    //poll cmd23 command done bit
				{
					if(int_mask & 0xb7c2) {
						printk(KERN_ERR "cmd 23 error bitmap EMMC_RINTSTS = 0x%x\n", int_mask);
						break;
					}
					else if(time_after_eq(jiffies, timeend)) {
						printk(KERN_ERR "cmd 23 poll cmd done timeout !!!\n");
						break;
					}
		...
		if(CMD_IDX_MASK(cmd_idx)==MMC_SET_BLOCK_COUNT)
		{/*Do Nothing, cmd23 does not use interrupt mode*/
		}
		else if (CMD_IDX_MASK(cmd_idx)==MMC_WRITE_MULTIPLE_BLOCK || CMD_IDX_MASK(cmd_idx)==MMC_WRITE_BLOCK )
		...
		if (emmc_port->card_busy_chk == true) {
			timeend = jiffies + msecs_to_jiffies(msec);
			while(time_before(jiffies, timeend))
			{
...
#ifdef CONFIG_ARCH_RTD139x
				if(CMD_IDX_MASK(cmd_idx)==MMC_READ_MULTIPLE_BLOCK && (get_rtd_chip_revision() < RTD_CHIP_A01))	//139x cmd18 has bug, we polling the dma length instead of dma done
...
#ifdef CONFIG_ARCH_RTD139x
					if(CMD_IDX_MASK(cmd_idx)==MMC_READ_MULTIPLE_BLOCK && (get_rtd_chip_revision() < RTD_CHIP_A01)) {
						if(readl(emmc_port->emmc_membase + EMMC_BYTCNT) == readl(emmc_port->emmc_membase + EMMC_TBBCNT)) {
...
//	rtkemmc_writel(1, emmc_port->emmc_membase + EMMC_SWC_SEL);
	...
	if (cmd_info->data)
		...
		make_ip_des((uintptr_t) data, block_count<<9, emmc_port->desc_paddr, emmc_port);
	else
		BUG_ON(1);
#else
	make_ip_des((uintptr_t) data, block_count<<9, emmc_port->desc_paddr, emmc_port);
...
static void rtkemmc_request(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if(mrq->cmd->opcode==MMC_SEND_EXT_CSD && !emmc_port->hs400_force_tuning)
	{
		...
		if((host->caps2 & MMC_CAP2_HS400_1_8V) && emmc_port->dqs_tuning==1 && emmc_port->emmc_tuning_addr!=0)    //bootcode doesn't do the hs400 tuning
		{
			...
			rtkemmc_send_cmd18(emmc_port, 512, emmc_port->emmc_tuning_addr/512+1024);       //we put the hs400 parameter data before tuning blk addr 1 block
	...
	*/
	...
	if ( emmc_port && cmd ){
		...
		if(mrq->sbc != NULL)
			rtkemmc_send_command(emmc_port, mrq->sbc);
...
static int rtkemmc_probe(struct platform_device *pdev)
{
	...
	if (prop) {
		...
		if (prop) {
			...
			printk(KERN_ERR "[%s] MBR format: emmc tuning offset start from 0x%x\n\n",__func__, emmc_port->emmc_tuning_addr);
		}
		else {
			emmc_port->emmc_tuning_addr = 0xa31000;	//use default value in the factory partition end block addr - 1025 blocks
			printk(KERN_ERR "[%s] Use Default tuning addr: emmc tuning offset start from 0x%x\n\n",__func__, emmc_port->emmc_tuning_addr);
		}
	}
