	...
	for (i=0; i < ARRAY_SIZE(rtk_wifi_sdio_clk_tbl); i++) {
		if (channel == rtk_wifi_sdio_clk_tbl[i].channel) {
			...
			if (value & 0x40000000)
				...
				writel(value | 0x40000000, crt_membase + 0x10a58);
#endif
			sdhci_writew(G_host, 0x107, SDHCI_CLOCK_CONTROL);
...
void write_swc(u32 addr, u32 val)
{
	struct arm_smccc_res res;
	arm_smccc_smc(0x8400ffff, addr, val, 0, 0, 0, 0, 0, &res);
}

u32 read_swc(u32 addr)
{
	struct arm_smccc_res res;
...
clock_set:
    ...
    if (real_div > 0x3e8) {         //the clock's lowest frequency should be 100k
...
void rtk_register_set(void)
{
	...
	if(get_SDIO_version()==4) {
		...
		if(readl(crt_membase+0x1a204)==0x00) {
			...
			writel(0x00b64388, crt_membase + 0x01A8); //SDIO 3.0, 208MHz
			mdelay(2);
...
void rtk_sdhci_platform_init(void)
{
	...
	udelay(200);
#if defined(CONFIG_ARCH_RTD13xx)
	reg_tmp = read_swc(0x9801c200);
	reg_tmp = reg_tmp & (0xfffff0ff);
	write_swc(0x9801c200, reg_tmp);
	reg_tmp = reg_tmp & 0x0;
	reg_tmp = read_swc(0x9801c208);
	reg_tmp = reg_tmp | (0x1 << 5);
	write_swc(0x9801c208, reg_tmp);
	reg_tmp = reg_tmp & 0x0;
	reg_tmp = read_swc(0x9800800c);
	reg_tmp = reg_tmp & (0xfff07fff);
	write_swc(0x9800800c, reg_tmp);
#endif
#endif
    }
    else {
	...
	writel(0x40000000, crt_membase+ 0x10a58);
#elif defined(CONFIG_ARCH_RTD13xx)
	writel(readl(crt_membase + 0x10A58) | 0x80000000,
		crt_membase + 0x10A58);
#endif
	writel(0x00000006, crt_membase + 0x01AC);
	writel(0x04517893, crt_membase + 0x01A4);
...
static int rtk_sdhci_change_rx_phase(struct sdhci_host *host, u8 sample_point)
{
...
static int rtk_sdhci_change_tx_phase(struct sdhci_host *host, u8 sample_point)
{
    unsigned int temp_reg = 0;
	u16 clk = 0;
...
static int rtk_sdhci_tuning_rx(struct sdhci_host *host)
{
    ...
    for(sample_point = 0 ; sample_point <= MAX_PHASE ; sample_point++){
        for(i = 0 ; i < TUNING_CNT ; i++){
            rtk_sdhci_change_rx_phase(host, (u8)sample_point);
    ...
    if(phase_map){
        ...
        rtk_sdhci_change_rx_phase(host, final_phase);
        ret = 0;
        goto out ;
    }else{
...
static int rtk_sdhci_tuning_tx(struct sdhci_host *host)
{
    ...
    for(sample_point = 0 ; sample_point <= MAX_PHASE ; sample_point++){
        for(i = 0 ; i < TUNING_CNT ; i++){
            rtk_sdhci_change_tx_phase(host, (u8)sample_point);
    ...
    if(phase_map){
        ...
        rtk_sdhci_change_tx_phase(host, final_phase);
        ret = 0;
        goto out ;
    }else{
        printk(KERN_ERR "%s  fail !phase_map\n", __func__);
	...
	if(readl(crt_membase+0x1a204)==0x00) {
		...
		if(get_SDIO_version()==3) {
			...
			writel(0x1d, crt_membase + 0x10a58);    //enlarge wait time for data
                        ...
                        writel(0x00000006, crt_membase + 0x01AC);
                        writel(0x04517893, crt_membase + 0x01A4);
                        writel(pll, crt_membase + 0x01A8); //SDIO 3.0, 200MHz
                        mdelay(2);
                        writel(0x00000007, crt_membase + 0x01AC);
			udelay(200);
#endif
