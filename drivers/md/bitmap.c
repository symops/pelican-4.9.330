...
__acquires(bitmap->lock)
{
	...
	if (mappage == NULL) {
		pr_debug("[KML] md/bitmap: map page allocation failed, hijacking\n");
...
out:
	if (ret)
		printk(KERN_ALERT "[KML] md: bitmap read error: (%dB @ %llu): %d\n",
			(int)PAGE_SIZE,
			(unsigned long long)index << PAGE_SHIFT,
			ret);
	return ret;
		  ...
		  int chunksize, int init)
{
	...
	if (bitmap->storage.file && !init) {
		pr_info("[KML] md: cannot resize file-based bitmap\n");
		return -EINVAL;
	}
